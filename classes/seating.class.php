...
class Seating extends Ticket {
	...
	/*function assignSeatNumber($bus_id, $travel_date, $num_of_seats)
	{
		...
		if (isset($details->id))
		{
			...
			# Book for seat
			self::$db->query("UPDATE boarding_bus SET booked_seats = CONCAT(booked_seats, ',', '$seat_no') WHERE id = '$details->id'");
			...
			$_SESSION['boarding_bus_id'] = self::$db->getLastInsertId();
			return '{"msg" : "0"}';
		}
	}*/
}
