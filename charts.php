...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$('#merge-routes').click(function() {
			...
			//alert("First route - " + seating[0] + " Second route " + seating[1]);
			// Prevent merging if both routes has bus details
			if (bus[0] != '' && bus[1] != '') {
				alert("The two selected routes has buses. Remove bus details from the bus that will not travel");
				return false;	
			}
			
			// Prevent merging for same route
			if (destination[1] == destination[0]) {
				alert("You can not merge buses going THE SAME ROUTE!");
				return false;
			}
			
			// Prevent merging for different buses (Luxury and mini)
			if ((seating[0] - seating[1] > 10) || (seating[1] - seating[0] > 10)) {
				alert("The two buses you are trying to merge are incompatible, Stop that nonsense!");
				return false;
			}
