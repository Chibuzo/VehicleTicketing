...
if (isset($_POST['add_vehicle'])) {
	$bus_type_id = $bus->addBusType($_POST['vehicle_name'], $_POST['num_of_seats']);

	// let's add fare for the new vehicle type on all the routes
	$route_ids = $route->getAllRouteIds();
	$fare = new Fare();
	foreach ($route_ids AS $routeID) {
		$fare->addSingleFare($routeID['id'], $bus_type_id, 0);
	}
}
					...
					<div class="box-body">
						<div>
							...
							<table class="table tablebordered table-striped">
								...
								<?php
									$html = ""; $n = 0;
									foreach ($bus->getAllBusTypes() AS $_bus) {
										$n++;
										$html .= "<tr>
													<td class='text-right'>$n</td>
													<td>{$_bus->name}</td>
